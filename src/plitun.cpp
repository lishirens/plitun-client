    ...
    , m_vpnConnected(false)
{
    ...
    setWindowIcon(QIcon(":/images/plitun64.png"));
#endif
...
void PliTun::afterShowOneTime()
{
    ...
    connect(rpc, &JsonRpcWebSocketClient::connected, this, [this]() { configVPN(); });
    ...
    rpc->callAsync("status", STATUS, [this](const QJsonValue &result) {
        ...
        if (!status.contains("code")) {
            ...
            if (!ui->buttonDetails->isEnabled()) {
                ...
                detailDialog->setRoutes(status["SplitExclude"].toArray(),
                                        status["SplitInclude"].toArray());
            }
        }
    });
}
