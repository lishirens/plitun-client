...
{
    ReadPasswordJob *readCredentialJob = new ReadPasswordJob(QLatin1String("keychain.plitun"));
...
{
    WritePasswordJob *writeCredentialJob = new WritePasswordJob(QLatin1String("keychain.plitun"));
    ...
    QObject::connect(writeCredentialJob, &QKeychain::WritePasswordJob::finished, [=]() {
        if (writeCredentialJob->error()) {
            emit error(
                tr("Write key failed: %1").arg(qPrintable(writeCredentialJob->errorString())));
            return;
        }
...
void KeyChainClass::deleteKey(const QString &key)
{
    ...
    QObject::connect(deleteCredentialJob, &QKeychain::DeletePasswordJob::finished, [=]() {
        if (deleteCredentialJob->error()) {
            emit error(
                tr("Delete key failed: %1").arg(qPrintable(deleteCredentialJob->errorString())));
            return;
        }
        emit keyDeleted(key);
    });
